...
MingwBackend::GenerateXmlBuildFilesMacro() const
{
	...
	while(!ProjectNode.xmlbuildfiles.empty())
	{
		XMLInclude xmlbuildfile = ProjectNode.xmlbuildfiles.top();
		ProjectNode.xmlbuildfiles.pop();
		...
		if (xmlbuildfile.fileExists) {
			...
			if ( numberOfExistingFiles % 5 == 4 || ProjectNode.xmlbuildfiles.empty())
			{
				fprintf ( fMakefile,
						  "\t%s",
						  xmlbuildFilenames.c_str ());
				if (ProjectNode.xmlbuildfiles.empty())
				{
					fprintf ( fMakefile, "\n" );
				}
				else
