...
PUNICODE_STRING	RegistryPath)		// path to the registry key
{
	...
	DbgPrint("Beginning initialization of SFsd in " __FUNCTION__ " of " __FILE__ "at " __SLINE__ "\n" );
	//- SFsdBreakPoint();
	DbgPrint("Past initial break point");
	...
	try {
		try {
         ...
         IoRegisterFileSystem(SFsdGlobalData.SFsdDeviceObject);
			DbgPrint("IoRegisterFileSystem successful.\n");
