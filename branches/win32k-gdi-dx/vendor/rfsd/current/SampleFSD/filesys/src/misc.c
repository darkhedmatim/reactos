...
#define			NO_ALLOCATION_FUNCTION		NULL
#define			NO_FREE_FUNCTION				NULL
#define			NO_FLAGS							0
#define			DEFAULT_LL_DEPTH				0

/*************************************************************************
*
...
void)
{
	...
	try {
		...
		// initialize each of the lookaside list headers ...
		ExInitializeNPagedLookasideList( &(SFsdGlobalData.ObjectNameLLHeader), 
			NO_ALLOCATION_FUNCTION, NO_FREE_FUNCTION, NO_FLAGS, 
			SFsdQuadAlign(sizeof(SFsdObjectName)), 'Ljbo', DEFAULT_LL_DEPTH);
		
		DbgPrint("Init ObjectName LL succeeded.\n");


		ExInitializeNPagedLookasideList( &(SFsdGlobalData.CCBLLHeader), 
			NO_ALLOCATION_FUNCTION, NO_FREE_FUNCTION, NO_FLAGS, 
			SFsdQuadAlign(sizeof(SFsdCCB)), 'Lbcc', DEFAULT_LL_DEPTH);
		
		DbgPrint("Init CCB LL succeeded.\n");

		
		ExInitializeNPagedLookasideList( &(SFsdGlobalData.FCBLLHeader), 
			NO_ALLOCATION_FUNCTION, NO_FREE_FUNCTION, NO_FLAGS, 
			SFsdQuadAlign(sizeof(SFsdFCB)), 'Lbcf', DEFAULT_LL_DEPTH);
		
		DbgPrint("Init FCB LL succeeded.\n");


		ExInitializeNPagedLookasideList( &(SFsdGlobalData.ByteLockLLHeader), 
			NO_ALLOCATION_FUNCTION, NO_FREE_FUNCTION, NO_FLAGS, 
			SFsdQuadAlign(sizeof(SFsdFileLockInfo)), 'Llyb', DEFAULT_LL_DEPTH);
		
		DbgPrint("Init ByteLock LL succeeded.\n");

		
		ExInitializeNPagedLookasideList( &(SFsdGlobalData.IrpContextLLHeader), 
			NO_ALLOCATION_FUNCTION, NO_FREE_FUNCTION, NO_FLAGS, 
			SFsdQuadAlign(sizeof(SFsdIrpContext)), 'Lpri', DEFAULT_LL_DEPTH);
		
		DbgPrint("Init IrpContext LL succeeded.\n");
...
void)
{
	try {
		...
		ExDeleteNPagedLookasideList(&(SFsdGlobalData.ObjectNameLLHeader));		
		ExDeleteNPagedLookasideList(&(SFsdGlobalData.CCBLLHeader));
		ExDeleteNPagedLookasideList(&(SFsdGlobalData.FCBLLHeader));
		ExDeleteNPagedLookasideList(&(SFsdGlobalData.ByteLockLLHeader));
		ExDeleteNPagedLookasideList(&(SFsdGlobalData.IrpContextLLHeader));
...
void)
{
   ...
   KIRQL							CurrentIrql;
	...
	PtrObjectName = (PtrSFsdObjectName) ExAllocateFromNPagedLookasideList(&(SFsdGlobalData.ObjectNameLLHeader));
...
PtrSFsdObjectName				PtrObjectName)
{
	...
	if (!(PtrObjectName->ObjectNameFlags & SFSD_OB_NAME_NOT_FROM_ZONE)) {
		// back to the zone
		KeAcquireSpinLock(&(SFsdGlobalData.ZoneAllocationSpinLock), &CurrentIrql);      
		ExFreeToNPagedLookasideList(&(SFsdGlobalData.ObjectNameLLHeader), PtrObjectName);
		KeReleaseSpinLock(&(SFsdGlobalData.ZoneAllocationSpinLock), CurrentIrql);
	} else {
...
void)
{
   ...
   KIRQL							CurrentIrql;
	...
	PtrCCB = (PtrSFsdCCB) ExAllocateFromNPagedLookasideList(&(SFsdGlobalData.CCBLLHeader));
...
PtrSFsdCCB						PtrCCB)
{
	...
	if (!(PtrCCB->CCBFlags & SFSD_CCB_NOT_FROM_ZONE)) {
		// back to the zone
		KeAcquireSpinLock(&(SFsdGlobalData.ZoneAllocationSpinLock), &CurrentIrql);
      ExFreeToNPagedLookasideList(&(SFsdGlobalData.CCBLLHeader), PtrCCB);
		KeReleaseSpinLock(&(SFsdGlobalData.ZoneAllocationSpinLock), CurrentIrql);
	} else {
		ExFreePool(PtrCCB);
	}
...
void)
{
   ...
   KIRQL							CurrentIrql;
	...
	PtrFCB = (PtrSFsdFCB) ExAllocateFromNPagedLookasideList(&(SFsdGlobalData.FCBLLHeader));
...
PtrSFsdFCB						PtrFCB)
{
	...
	if (!(PtrFCB->FCBFlags & SFSD_FCB_NOT_FROM_ZONE)) {
		// back to the zone
		KeAcquireSpinLock(&(SFsdGlobalData.ZoneAllocationSpinLock), &CurrentIrql);
      ExFreeToNPagedLookasideList(&(SFsdGlobalData.FCBLLHeader), PtrFCB);
		KeReleaseSpinLock(&(SFsdGlobalData.ZoneAllocationSpinLock), CurrentIrql);
	} else {
		ExFreePool(PtrFCB);
	}
...
void)
{
   ...
   KIRQL							CurrentIrql;
	...
	PtrByteLocks = (PtrSFsdFileLockInfo) ExAllocateFromNPagedLookasideList(&(SFsdGlobalData.ByteLockLLHeader));
...
PtrSFsdFileLockInfo					PtrByteLocks)
{
	...
	if (!(PtrByteLocks->FileLockFlags & SFSD_BYTE_LOCK_NOT_FROM_ZONE)) {
		// back to the zone
		KeAcquireSpinLock(&(SFsdGlobalData.ZoneAllocationSpinLock), &CurrentIrql);
      ExFreeToNPagedLookasideList(&(SFsdGlobalData.ByteLockLLHeader), PtrByteLocks);
		KeReleaseSpinLock(&(SFsdGlobalData.ZoneAllocationSpinLock), CurrentIrql);
	} else {
...
PDEVICE_OBJECT		PtrTargetDeviceObject)
{
   ...
   KIRQL							CurrentIrql;
	...
	PtrIrpContext = (PtrSFsdIrpContext) ExAllocateFromNPagedLookasideList(&(SFsdGlobalData.IrpContextLLHeader));
//?
	// release the spinlock
	KeReleaseSpinLock(&(SFsdGlobalData.ZoneAllocationSpinLock), CurrentIrql);	
	...
	if (Irp) {
		...
		DbgPrint("AllocateIrpContext with major %x and minor %x", PtrIoStackLocation->MajorFunction, PtrIoStackLocation->MinorFunction);
		...
		try {
			// From http://www.osronline.com/lists_archive/ntfsd/thread1966.html
			// IoIsOperationSynchronous will die if there is no file object in the
			// IRP's current stack location. I believe this may be the case with IRP_MJ_SHUTDOWN.

			if (IoIsOperationSynchronous(Irp)) {  
				SFsdSetFlag(PtrIrpContext->IrpContextFlags, SFSD_IRP_CONTEXT_CAN_BLOCK);
			}
		}		
		except( EXCEPTION_EXECUTE_HANDLER ) {		// Intended to catch SYSTEM_THREAD_EXCEPTION_NOT_HANDLED =  7e?
			NTSTATUS RC = GetExceptionCode();
			DbgPrint("[WARN] Exception 0x%x in IoIsOperationSynchronous of SFsdAllocateIrpContext\n", RC);
		}
...
PtrSFsdIrpContext					PtrIrpContext)
{
	...
	if (!(PtrIrpContext->IrpContextFlags & SFSD_IRP_CONTEXT_NOT_FROM_ZONE)) {
		// back to the zone
		KeAcquireSpinLock(&(SFsdGlobalData.ZoneAllocationSpinLock), &CurrentIrql);
      ExFreeToNPagedLookasideList(&(SFsdGlobalData.IrpContextLLHeader), PtrIrpContext);
		KeReleaseSpinLock(&(SFsdGlobalData.ZoneAllocationSpinLock), CurrentIrql);
	} else {
