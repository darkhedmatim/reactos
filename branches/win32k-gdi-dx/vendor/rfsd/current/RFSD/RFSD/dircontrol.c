 ...
 * FILE:             dirctl.c
 * PROGRAMMER:       Mark Piper, Matt Wu <mattwu@163.com>
			...
			IN PVOID		 pPreviousEntry	)						// A pointer to the previous dir entry in Buffer, which will be linked into the newly added entry if one is created
{
	...
	if (pPreviousEntry) {
		...
		((PFILE_NAMES_INFORMATION) (pPreviousEntry))->NextEntryOffset = 
			(PUCHAR) Buffer + UsedLength - (PUCHAR) (pPreviousEntry);
	}
...
RfsdQueryDirectory (IN PRFSD_IRP_CONTEXT IrpContext)
{
    ...
    __try {
		...
		DbgPrint("QueryDirectory on Key {%x,%x,%x,%x}\n", 
			...
			PVOID			pContext)
{
	...
	{ 
		DbgPrint("Block %i did not contain the appropriate diritem header\n", BlockNumber);
