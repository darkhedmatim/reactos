...
NTSTATUS
Ext2QueryVolumeExtents(PEXT2_FILESYS Ext2Sys)
{
	ULONG	dataSize = 1024;
...
QueryExtent:
    ...
    status = NtDeviceIoControlFile(
                Ext2Sys->MediaHandle, NULL, NULL, NULL, &ioSb,
...
Ext2GetMediaInfo( PEXT2_FILESYS Ext2Sys )
{
    ...
    Ext2QueryVolumeExtents(Ext2Sys);
    ...
    if (!NT_SUCCESS(Status))
    {
        printf("Mke2fs:  Failed to query disk gemetry, rc=%xh\n", Status);
        goto errorout;
    }
    ...
    if (!NT_SUCCESS(Status))
    {
        printf("Mke2fs:  Failed to query partition info, rc=%xh\n", Status);
        goto errorout;
    }
...
Ext2LockVolume( PEXT2_FILESYS Ext2Sys )
{
    ...
    if (!NT_SUCCESS(Status))
    {
        printf("Mke2fs: Error when locking volume: Status = %xh %s...\n",
...
Ext2UnLockVolume( PEXT2_FILESYS Ext2Sys )
{
    ...
    if (!NT_SUCCESS(Status))
    {
        printf("Mke2fs: Error when unlocking volume ...\n");
        goto errorout;
    }
...
Ext2DisMountVolume( PEXT2_FILESYS Ext2Sys )
{
    ...
    if (!NT_SUCCESS(Status))
    {
        printf("Mke2fs: Error when dismounting volume ...\n");
        goto errorout;
    }
                 ...
                 PUCHAR        DeviceName )
{
    ...
    memset(AnsiName, 0, sizeof(UCHAR) * 256);
    ...
    //
    //RtlInitUnicodeString(&UnicodeFilespec, L"\\DosDevices\\C:\\BSOD-EA.txt");
    //RtlInitUnicodeString(&UnicodeFilespec, L"\\Device\\Harddisk0\\Partition2");
    //RtlInitUnicodeString(&UnicodeFilespec, L"\\DosDevices\\C:");

    //RtlInitUnicodeString(&UnicodeFilespec, DeviceName);
    ...
    if( !NT_SUCCESS(Status) )
    {
        printf("Mke2fs: Create system service failed status = 0x%0x\n", Status);
    ...
    {
        printf("Mke2fs: Create failed with status = 0x%0x\n",Iosb.Status);
        return Status;
    }
