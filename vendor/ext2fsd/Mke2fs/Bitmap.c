...
bool ext2_allocate_block_bitmap(PEXT2_FILESYS Ext2Sys)
{
    ...
    Ext2Sys->block_map = (PEXT2_BLOCK_BITMAP)malloc(sizeof(EXT2_BLOCK_BITMAP));

    if (!Ext2Sys->block_map)
    {
        printf("Mke2fs: error allocating block bitmap...\n");
        return false;
    }
    ...
    Ext2Sys->block_map->bitmap = (char *)malloc(size);

    if (!Ext2Sys->block_map->bitmap)
    {
        free(Ext2Sys->block_map);
...
bool ext2_allocate_inode_bitmap(PEXT2_FILESYS Ext2Sys)
{
    ...
    Ext2Sys->inode_map = (PEXT2_INODE_BITMAP)malloc(sizeof(EXT2_INODE_BITMAP));

    if (!Ext2Sys->inode_map)
    {
        printf("Mke2fs: error allocating inode bitmap...\n");
        return false;
    }
    ...
    Ext2Sys->inode_map->bitmap = (char *)malloc(size);

    if (!Ext2Sys->inode_map->bitmap)
    {
        free(Ext2Sys->inode_map);
...
bool ext2_write_inode_bitmap(PEXT2_FILESYS fs)
{
    ...
    bitmap_block = (char *) malloc(fs->blocksize);
