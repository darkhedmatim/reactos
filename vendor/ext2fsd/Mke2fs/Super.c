...
void ext2_print_super(PEXT2_SUPER_BLOCK pExt2Sb)
{
    ...
    printf("     Mount Time: %s", ctime((time_t *) & (pExt2Sb->s_mtime)));
    printf("     Write Time: %s", ctime((time_t *) & (pExt2Sb->s_wtime)));
    ...
    printf("     Last Check: %s", ctime((time_t *) & (pExt2Sb->s_lastcheck)));
    ...
    printf("     uuid = ");
    for (i=0; i < 16; i++)
        printf("%x ", pExt2Sb->s_uuid[i]);
    printf("\n");

    printf("     volume label name: ");
    for (i=0; i < 16; i++)
    {
        if (pExt2Sb->s_volume_name[i] == 0)
            break;
        printf("%c", pExt2Sb->s_volume_name[i]);
    }
    printf("\n");
...
bool ext2_initialize_sb(PEXT2_FILESYS Ext2Sys)
{
    ...
    pExt2Sb->s_mtime = pExt2Sb->s_wtime = ext2_unix_time(SysTime.QuadPart);

    pExt2Sb->s_lastcheck = ext2_unix_time(SysTime.QuadPart);

    if (!pExt2Sb->s_blocks_per_group) {
        pExt2Sb->s_blocks_per_group = Ext2Sys->blocksize * 8;
    }
    if (pExt2Sb->s_blocks_per_group >= 0x10000) {
        pExt2Sb->s_blocks_per_group = 0xFFF0;
    }
    ...
    if (!pExt2Sb->s_inodes_count) {
        count.QuadPart  = (LONGLONG)(pExt2Sb->s_blocks_count);
        count.QuadPart *= Ext2Sys->blocksize;
        count.QuadPart /= average_fsize;
        if (count.HighPart) {
            pExt2Sb->s_inodes_count = (ULONG)(-1);
        } else {
            pExt2Sb->s_inodes_count = count.LowPart;
        }
    }
...
retry:
    ...
    if (pExt2Sb->s_inodes_per_group >= 0x10000) {
        pExt2Sb->s_inodes_per_group = 0xFFF0;
        count.QuadPart = (LONGLONG)pExt2Sb->s_blocks_count;
        count.QuadPart *= pExt2Sb->s_inodes_per_group;
        count.QuadPart /= pExt2Sb->s_inodes_count;
        pExt2Sb->s_blocks_per_group = count.LowPart & (~7);
        goto retry;
    }
