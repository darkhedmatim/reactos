...
bool write_inode_tables(PEXT2_FILESYS fs)
{
    ...
    for (i = 0; i < fs->group_desc_count; i++)
    {
        ...
        retval = zero_blocks(fs, blk, num, &blk, (ULONG *)&num);
        if (!retval)
        {
            printf("Mke2fs: Could not write %u blocks "
                "in inode table starting at %u.\n"),
                num, blk;

            zero_blocks(0, 0, 0, 0, 0);
            return false;
        }
        printf("     %2.2u%% percent is done.\r", i * 100 / fs->group_desc_count);
    }

    zero_blocks(0, 0, 0, 0, 0);

    printf("Writing inode table information done! \n\n");
